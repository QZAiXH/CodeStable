---
name: cs-loop
description: 搭建和运行 CodeStable 上的 loop engineering 控制层，用 decision-codex 起草低等级推进判断、approval-codex 独立审批、worker-codex 实际执行；支持先判断任务大小，单 feature 走 cs-feat，大需求走 cs-roadmap 拆解、审核后逐条进入 cs-feat。触发：用户说 loop engineering、自动循环、多个 Codex 配合、低等级决策代理、让 Codex 自己推进但保留人类拍板、自动拆 roadmap 并推进子 feature。
---
...
开始任何判断或动作前，先读取 `.codestable/attention.md`；缺失则视为骨架不完整，提示先运行 `cs-onboard`。再读取 `.codestable/reference/system-overview.md` 和本技能 `reference.md`。目标是新功能 / 大需求时，还要读取本技能 `roadmap-loop.md`。
...
| 场景 | workflow |
|---|---|
| 新功能 / 需求变更 | 先判断任务大小：单 feature → `cs-feat`；多 feature / 跨模块契约 / DAG → `cs-roadmap` |
...
- `active_workflow`
- roadmap 场景还要写 `roadmap` / `roadmap_item` / `roadmap_stage`
- 本轮对应的 CodeStable 产物路径，例如 feature design / checklist、issue analysis / fix-note
- 本轮只允许执行哪一个阶段
- 本轮完成后要回写哪个 CodeStable 产物
- 新 roadmap 子任务起步时必须有 `Context Boundary` 和 `Previous Subtask Summary`
...
详细规则看 `roadmap-loop.md`。核心流程：

1. decision-codex 对新功能先做 size routing：一条 feature 能闭环就走 `cs-feat`；需要多个可独立验收交付、跨模块接口契约或依赖 DAG 时走 `cs-roadmap`
2. 走 `cs-roadmap` 时，第一段 worker 只创建 / 更新 roadmap 主文档和 items.yaml，不写业务代码
3. approval-codex 作为拆解审核者，检查模块边界、接口契约、items DAG、依赖理由、最小闭环；可修问题 `REVISE`，真正需要产品 / 架构拍板才 `ESCALATE`
4. roadmap 拆解通过后，decision-codex 每轮只启动一个 `planned` 且依赖已完成的 item，交给 `cs-feat-design` 的"从 roadmap 条目起头"入口
5. 每个 item 都完整走 `cs-feat-design → cs-feat-impl → cs-feat-accept`；accept 回写 roadmap 后，下一轮才选下一条
6. 新 item 启动时必须隔离上下文：worker 只读 roadmap、目标 item、目标 feature 产物、当前代码和上一 item 的短摘要，不继承上一 item 的 feature 目录 / worker 输出
...
- 对新功能做 size routing，选择 `cs-feat` 或 `cs-roadmap`
- 在 roadmap 拆解通过后选择下一条可启动 item
...
- 检查 roadmap 拆解是否合理，或新 item 的 context boundary 是否足够窄
...
- 新 roadmap item 开始时，把 `Context Boundary` 当硬边界，只按 `Previous Subtask Summary` 接收上一轮必要信息
- 小步修改，跑 brief 指定验证
- 不能自行改变目标、范围、架构或产品语义
...
- `subtask-summary.md`：roadmap 子任务之间只传递的短摘要，不承载完整上下文
- `runs/`：每轮原始输出归档
...
如果 `.codestable/loops/` 不存在，创建它。为本次目标创建 `YYYY-MM-DD-{slug}` 目录，并按 `reference.md` 模板写入控制文件。脚本支持 `--init`，只初始化 / 修复 loop 目录，不运行 Codex。
...
脚本会在缺失时生成 `loop.md`，但其中 `TBD` 只是占位；自动推进前必须让 decision-codex 或用户把目标、范围、停止条件和验证证据补实。
...
approval-codex 不负责想新方案。它只回答：这个 proposal 能不能在当前 CodeStable 证据下被低风险放行。脚本还会校验 human escalation 模板，缺关键章节时不继续执行。
...
worker-codex 读取 `worker-brief.md`，执行其中的步骤。脚本会先校验 brief 必须含 `Task` / `Active Workflow` / `Inputs` / `Allowed Changes` / `Verification` / `Return Format` 和具体 CodeStable 产物路径。worker 可以改代码和 CodeStable 产物，但只能在 brief 范围内改。完成后在最终输出里写：
...
脚本会从 worker 输出提取 `verification result` 写入 `state.yaml.last_verification`；若 worker 报 blocker，会记录 blocker 签名和次数，同一 blocker 连续两轮自动写 `human-escalation.md` 并停到 `waiting-human`。
