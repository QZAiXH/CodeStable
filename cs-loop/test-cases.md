...
| B02 | 首次运行创建空日志 | loop 目录缺日志文件 | 创建 `decision-log.md`、`approval-log.md`、`worker-brief.md`、`human-escalation.md`、`human-decision.md` |
...
| D10 | 无 active workflow | `CONTINUE` 但无 workflow | 理想期望是 approval `REVISE`；当前脚本只透传给 approval，需要补用例暴露风险 |
...
| F09 | worker 失败 | 当前脚本因 `set -e` 中断；建议记录 worker 输出或 blocker 后再退出 |
| F10 | 连续两轮同 blocker | 符合守护规则，应升级 human；当前脚本未自动判断，适合作为模型行为验收用例 |
...
| I09 | worker brief 边界 | 必须含 `Active Workflow`、`Inputs`、`Allowed Changes`、`Verification` |
...
| ID | 场景 | 期望 |
|---|---|---|
| K01 | 每轮 decision | `decision-log.md` 追加时间戳、runs 输出路径、Status |
| K02 | 每轮 approval | `approval-log.md` 追加时间戳、runs 输出路径、Reviewed decision、Status |
| K03 | worker 输出 | `runs/*-worker-codex.md` 保留原始输出 |
| K04 | `worker-brief.md` | `CONTINUE` 时被 decision 输出替换 |
| K05 | `human-escalation.md` | ESCALATE 或不可解析 approval 时写入可读上下文 |
| K06 | 运行多轮 | 历史 log 追加，不覆盖旧记录 |
| K07 | 时间戳唯一性 | 同一秒内多次运行可能覆盖 runs 文件；建议新增测试暴露风险，必要时加入 PID 或纳秒 |
...
1. worker 失败时的状态记录：当前 `set -e` 会直接中断，缺少 `state.yaml` 和 log 层面的失败记录。
2. 同秒多次运行的 runs 文件覆盖风险。
3. `CONTINUE` 但缺 `Active Workflow` 或 CodeStable artifact path 的硬阻断。
4. `DONE` 但无验证证据时 approval 必须 `REVISE` 或 `ESCALATE`。
5. 高风险场景的端到端模型验收：产品语义、架构、安全、数据风险必须升级 human。
6. 多轮 loop 的真实闭环：`CONTINUE -> worker -> decision -> DONE`。
