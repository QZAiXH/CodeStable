...
CodeStable 把开发活动建模成 **7 个实体 + 3 个流程**，所有产物聚在 `.codestable/`；需要自动推进时，可在这些实体之上叠加 `loops/` 控制层：
...
└── loops/           loop engineering 控制层（decision-codex + approval-codex + worker-codex）
```
...
| loop engineering / 自动推进 / 多个 Codex 配合 / 低等级决策代理 | `cs-loop`（decision-codex 只读起草，approval-codex 独立审批，worker-codex 实际执行） |
| 补 / 更新需求文档 | `cs-req` |
...
- 自动推进"让多个 Codex 替我跑闭环但保留人类拍板" → `cs-loop`
