...
当前仓库会被 `npx skills add` 识别出 27 个可安装 skills。想一次性拉取所有 CodeStable skills，但仍保留 agent / 安装范围选择，用：

```bash
npx skills add https://github.com/QZAiXH/CodeStable --skill '*'
```

如果要非交互地安装全部 skills 到全部支持的 agent，用：

```bash
npx skills add https://github.com/QZAiXH/CodeStable --all
```

安装后，先在目标项目里接入 CodeStable：
...
我在开发一套新的 Harness Agent 时，一开始当然是 VibeCoding——我只写设计和需求，代码由 AI 来改。这样支撑了大部分特性的开发。直到有一天 Codex 反复解决不了一个我认为比较简单的问题，并且反复在同一个地方犯错。我就知道项目需要一套工作流来维持它继续进行了。
...
CodeStable 顺着软件编码的真实流程来设计，把开发活动建模成 **7 个实体** 和 **3 个流程**。
...
| **审计** | audit | 主动扫描系统风险、架构偏离和维护债，只产出发现清单，不直接修 |
...
<tr><td><b>系统审计</b></td><td><code>cs-audit</code></td><td>主动扫描 bug 隐患、安全风险、性能问题、维护债和架构偏离，只列发现清单</td></tr>
<tr><td rowspan="4"><b>知识沉淀</b></td><td><code>cs-learn</code></td><td>把踩过的坑 / 好做法沉淀成 learning 文档</td></tr>
...
<tr><td><code>cs-note</code></td><td>把一两行每次启动都要知道的项目注意事项写入 <code>.codestable/attention.md</code></td></tr>
...
<tr><td><b>浏览器工具</b></td><td><code>browser-bridge</code></td><td>通过浏览器扩展控制真实页面，执行自动化、数据抽取和组件证据采集</td></tr>
</table>
...
你的项目/
├── .codestable/
│   ├── attention.md                       # 所有 CodeStable skill 启动时先读的项目注意事项
│   │
...
- `audits/` 只记录主动扫描出来的发现清单，具体修复再路由到 issue 或 refactor 流程
- `attention.md` 是短而稳定的项目注意事项入口，由 `cs-note` 维护
...
[![Star History Chart](https://api.star-history.com/chart?repos=QZAiXH/CodeStable&type=date&legend=top-left)](https://www.star-history.com/?repos=QZAiXH%2FCodeStable&type=date&legend=top-left)
